...
from utils import esc, back_keyboard, confirm_keyboard, get_account
...
async def budget_receive_amount(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(
            ...
            f"תקציב יומי חדש ל{entity_type}: <b>${amount:.2f}</b>\n\n"
            f"האם לאשר?",
            parse_mode="HTML",
...
async def budget_confirm(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("⏳ מעדכן תקציב...", parse_mode="HTML")
    ...
    try:
        ...
        if success:
            ...
            await callback.message.edit_text(
                f"✅ <b>תקציב {entity_label} עודכן ל-${amount_display:.2f}/יום</b>",
                parse_mode="HTML",
