...
from utils import accounts_keyboard, report_period_keyboard, back_keyboard, format_currency, format_number, format_percent, get_account, esc
...
async def report_show(callback: CallbackQuery):
    ...
    try:
        ...
        text = (
            ...
            f"💰 <b>הוצאה:</b> {esc(spend)}\n"
            f"👁️ <b>חשיפות:</b> {esc(impressions)}\n"
            f"🖱️ <b>קליקים:</b> {esc(clicks)}\n"
            f"📈 <b>CTR:</b> {esc(ctr)}\n"
            f"💵 <b>CPC:</b> {esc(cpc)}\n"
            f"🎯 <b>טווח הגעה:</b> {esc(reach)}\n"
            f"🔁 <b>תדירות:</b> {esc(freq)}\n\n"
...
def build_daily_report_text(report_data: dict, acc_name: str) -> str:
    if "error" in report_data:
        return f"❌ שגיאה בטעינת דוח עבור {esc(acc_name)}: {esc(report_data['error'])}"
    ...
    return (
        f"📊 <b>דוח יומי — {esc(acc_name)}</b>\n\n"
        f"💰 הוצאה: {esc(spend)}\n"
        f"👁️ חשיפות: {esc(impressions)}\n"
        f"🖱️ קליקים: {esc(clicks)}\n"
        f"📈 CTR: {esc(ctr)}\n"
        f"💵 CPC: {esc(cpc)}\n"
